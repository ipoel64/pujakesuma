-- Drop old policies that check auth.jwt() ->> 'user_role'
...
CREATE POLICY "Petugas can read own keluarga records" ON public.keluarga
    FOR SELECT TO authenticated USING (
        petugas_id = auth.uid() 
        AND EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'petugas')
    );
...
CREATE POLICY "Pengawas can read all keluarga records" ON public.keluarga
    FOR SELECT TO authenticated USING (
        EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'pengawas')
    );
